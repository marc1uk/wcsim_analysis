...
TFile* mrdfile = TFile::Open(mrdinfile, "READ");
if (mrdfile->IsZombie()) {
...
TFile* fullfile = TFile::Open(fullinfile, "READ");
...
//GenerateVectors produces two vectors, mrdparticleds and mrdprocessids. each is a vector of pair<string,int>, where the string is the particle/process name and the int is the number of occurrances.
GenerateVectors(mrdtree,mrdparticleids,mrdprocessids);


//GeneratePieCharts produces two pie charts of the above vectors, showing the distribution of particles and processes that occurred.
GeneratePieCharts(mrdparticleids, mrdprocessids);

//MakeEnergyDepPlots produces a histogram of the energy deposition of a hit in the MRD, with sub-histograms for each combination of particle and process. 
MakeEnergyDepPlots(mrdtree, mrdparticleids, mrdprocessids);

//PlotPenetration just plots a histogram of the penetration of tracks (assuming they all start at 0...) with a histogram binning such that each bar is a layer
PlotPenetration(mrdtree);

//EnergyDeposition is fairly self-explanatory. 
...
void GenerateVectors(TTree* mrdtree, stringintpairvec &mrdparticleids, stringintpairvec &mrdprocessids){
TCanvas* vectCanv = new TCanvas("vectCanv","Title");
...
TCanvas* stackCanv = new TCanvas("stackCanv","Title");
...
// =============================================================================
TCanvas* penetrCanv = new TCanvas("penetrCanv","Title");
penetrCanv->cd();
...
//3) get z position branch into that array
//4) scan through the array and increment zmax found so far.
TCanvas* edeposCanv = new TCanvas("edeposCanv","Title");
Int_t numentries = mrdtree->GetEntries();
TBranch* numhitsbranch = mrdtree->GetBranch("hitnum");
Int_t arraysize = 0;
numhitsbranch->SetAddress(&arraysize);
...
*/
TCanvas* edeposCanv2 = new TCanvas("edeposCanv2","Title",8,59,1266,737);
...
TCanvas* edeposCanv22 = new TCanvas("edeposCanv22","Title");
...
for(Int_t entry=0;entry<numentries;entry++){
	...
	if(eventduration<maxtrackduration){
		...
		/* TODO: ADD TRACK START TIMES TO RECOTREE, ALONG WITH VECTOR OF HITS IN THIS TRACK? PANEL, PADDLE, PMT, EDEP, ORIGINALHITNUM.*/
...
// a 'hit' on a PMT is just one photon - a given paddle 'hit' will generate many PMT hits. Call the paddle hit a 'strike' - it will cause many PMT hits, all on the same PMT, only differing in time. 
...
// ======================================
TCanvas* countCanv = new TCanvas("countCanv","Muon Counts");
countCanv->cd();
