...
WCSimRootTrigger* tr;
std::vector<TVector3> allvs; for(int i=0; i<t->GetEntries(); i++){evb->GetEntry(i); tr=ev->GetTrigger(0); TVector3 avec(tr->GetVtx(0),tr->GetVtx(1),tr->GetVtx(2)); allvs.push_back(avec);}
...
for(int i=0;i<allvs.size();i++){TVector3 avec=allvs.at(i); ahist.Fill(avec.X(), avec.Y(), avec.Z()); xplot.Fill(avec.X()); yplot.Fill(avec.Y()); zplot.Fill(avec.Z());}
