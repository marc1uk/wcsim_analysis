...
b->GetEntry(0);
WCSimRootTrigger* firsttrig=e->GetTrigger(0); // cherenkov hits for ALL triggers are stored in trigger 0!
WCSimRootEventHeader* h=firsttrig->GetHeader();
h->GetDate();

int maxentriestoprint=100;
int maxtriggerstoprint=10;
int maxprimariestoprint=0; // always 1
...
for(int i=0; i<min(maxentriestoprint,(int)b->GetEntries()); i++){
  ...
  for(int j=0; j<min(maxtriggerstoprint,(int)e->GetNumberOfEvents()); j++){
    ...
    h=r->GetHeader();
    std::cout<<"Trigger time was "<<h->GetDate()<<std::endl;
    int nprimaries = 1; //r->GetNvtxs();	// always 1, 0 for delayed triggers
    ...
    for(int primaryi=0; primaryi<min(maxprimariestoprint,nprimaries); primaryi++){
      ...
      cout<<"event "<<i<<" trigger "<<j<<" primary track "<<primaryi<<" was at ("
      ...
      for(int digiti=0; digiti<min(maxdigitstoprint,(int)ndigits); digiti++){
        ...
        for(int photoni=0; photoni<min(maxphotonsperdigittoprint,(int)truephotonindices.size()); photoni++){
          ...
          WCSimRootCherenkovHitTime *thehittimeobject = 
            (WCSimRootCherenkovHitTime*)firsttrig->GetCherenkovHitTimes()->At(thephotonsid); // XXX first trig!
...
//        lastdigittime=digihit->GetT();
      ...
      for(int photoni=0; photoni<min(maxphotonstoprint,(int)numphotons); photoni++){
        WCSimRootCherenkovHitTime *thehittimeobject = 
          (WCSimRootCherenkovHitTime*)firsttrig->GetCherenkovHitTimes()->At(photoni); // XXX first trig!
        Int_t thephotonsparenttrackid = thehittimeobject->GetParentID();
        cout<<"    photon "<<photoni<<" has parent "<<thephotonsparenttrackid<<endl;
      }
      
    }  // loop over primaries (only ever 1 neutrino vertex)
  std::cout<<"moving to next subevent"<<std::endl;
  } // loop over subevents
} // loop over events
